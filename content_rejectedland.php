...
// Check if the user is a content
...
try {
    // Fetch records with status 'rejectedC'
    $sql = "SELECT * FROM content WHERE status = 'rejectedC'";
    $stmt = $pdo->query($sql);
    $records = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if (!$records) {
        echo "<p>No records waiting for review.</p>";
        exit;
    }
    ...
    foreach ($clients as $clientName => $months) {
        ...
        foreach ($months as $month => $records) {
            ...
            echo '<form action="content_rejected.php" method="post" target="_blank" onsubmit="console.log(\'Form submitted for ' . htmlspecialchars($clientName) . ' in ' . htmlspecialchars($month) . '\');">';
            ...
            echo '<button type="submit" class="review-btn">Review and Rewrite</button>';
            echo '</form>';
